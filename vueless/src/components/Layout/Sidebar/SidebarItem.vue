    ...
    <template v-for="item in routes">
      ...
      <router-link v-if="!item.children || item.children.length <= 0" :to="item.path"
        ...
        <el-menu-item :index="item.path" :class="{'submenu-title-noDropdown':!isNest}">
          ...
          <span slot="title">{{i18nAdapt(item.name)}}</span>
        </el-menu-item>
      </router-link>
      ...
      <el-submenu v-else :index="item.redirect" :key="item.name">
        <template slot="title">
          <v-icon v-if="item.icon" :icon-class="item.icon"></v-icon>
          <span v-if="item.name" slot="title">{{i18nAdapt(item.name)}}</span>
        </template>
        ...
        <template v-for="child in item.children">
          ...
          <router-link :to="child.path" :key="child.name">
            <el-menu-item :index="child.path">
              ...
              <span v-if="child.name" slot="title">{{i18nAdapt(child.name)}}</span>
            </el-menu-item>
          </router-link>
        </template>
      </el-submenu>
